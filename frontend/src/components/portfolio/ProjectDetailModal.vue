      ...
      <div class="flex items-center justify-center min-h-screen p-4 text-center sm:p-0">
        ...
        >
          ...
          <div class="px-6 pt-6 pb-8">
            ...
            <!-- Link zum Erfolg wenn vorhanden -->
            <div v-if="project.erfolgId" class="mt-6 pt-6 border-t border-white/10">
              <button
                @click="scrollToErfolg"
                class="w-full px-6 py-3 bg-blue-600 hover:bg-blue-700 border border-blue-500 rounded-lg text-white font-medium transition-all duration-300 flex items-center justify-center gap-2 group"
              >
                ...
                <span>{{ $t('projects.modal.viewErfolg') }}</span>
                <svg
...
export default defineComponent({
  ...
  methods: {
    ...
    scrollToErfolg() {
      if (!this.project.erfolgId) return

      // Schließe das Modal
      this.close()

      // Warte kurz bis Modal geschlossen ist
      setTimeout(() => {
        // Scrolle zur Erfolge-Sektion
        const erfolgeSection = document.getElementById('erfolge')
        if (erfolgeSection) {
          erfolgeSection.scrollIntoView({ behavior: 'smooth', block: 'start' })

          // Warte bis Scroll-Animation fertig ist, dann highlighte den Erfolg
          setTimeout(() => {
            const erfolgElement = document.querySelector(
              `[data-erfolg-id="${this.project.erfolgId}"]`,
            )
            if (erfolgElement) {
              erfolgElement.scrollIntoView({ behavior: 'smooth', block: 'center' })

              // Kurze Flash-Animation
              erfolgElement.classList.add('erfolg-highlight')
              setTimeout(() => {
                erfolgElement.classList.remove('erfolg-highlight')
              }, 2000)
            }
          }, 500)
        }
